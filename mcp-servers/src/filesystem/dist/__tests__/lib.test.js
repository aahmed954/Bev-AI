...
formatSize, normalizeLineEndings, createUnifiedDiff, 
// Security & validation functions
validatePath, setAllowedDirectories, 
// File operations
getFileStats, readFileContent, writeFileContent, 
// Search & filtering functions
searchFilesWithValidation, 
// File editing functions
applyFileEdits, tailFile, headFile } from '../lib.js';
// Mock fs module
...
describe('Lib Functions', () => {
    ...
    describe('Pure Utility Functions', () => {
        ...
        describe('createUnifiedDiff', () => {
            ...
            it('handles identical content', () => {
                ...
                expect(diff.split('\n').filter((line) => line.startsWith('+') && !line.startsWith('+++'))).toHaveLength(0);
                expect(diff.split('\n').filter((line) => line.startsWith('-') && !line.startsWith('---'))).toHaveLength(0);
            });
    ...
    describe('Security & Validation Functions', () => {
        describe('validatePath', () => {
            ...
            beforeEach(() => {
                mockFs.realpath.mockImplementation(async (path) => path.toString());
            });
            ...
            it('rejects when parent directory does not exist', async () => {
                ...
                const enoentError1 = new Error('ENOENT');
                enoentError1.code = 'ENOENT';
                const enoentError2 = new Error('ENOENT');
    ...
    describe('Search & Filtering Functions', () => {
        describe('searchFilesWithValidation', () => {
            ...
            it('excludes files matching exclude patterns', async () => {
                ...
                mockFs.realpath.mockImplementation(async (inputPath) => {
                    const pathStr = inputPath.toString();
                    // Return the path as-is for validation
                    return pathStr;
                });
                const result = await searchFilesWithValidation(testDir, '*test*', allowedDirs, { excludePatterns: ['*.log', 'node_modules'] });
            ...
            it('handles validation errors during search', async () => {
                ...
                mockFs.realpath.mockImplementation(async (path) => {
            ...
            it('handles complex exclude patterns with wildcards', async () => {
                ...
                const testDir = process.platform === 'win32' ? 'C:\\allowed\\dir' : '/allowed/dir';
                const allowedDirs = process.platform === 'win32' ? ['C:\\allowed'] : ['/allowed'];
                const result = await searchFilesWithValidation(testDir, '*test*', allowedDirs, { excludePatterns: ['*.backup'] });
    ...
    describe('File Editing Functions', () => {
        describe('applyFileEdits', () => {
            ...
            it('applies simple text replacement', async () => {
                ...
                expect(mockFs.writeFile).toHaveBeenCalledWith(expect.stringMatching(/\/test\/file\.txt\.[a-f0-9]+\.tmp$/), 'line1\nmodified line2\nline3\n', 'utf-8');
                expect(mockFs.rename).toHaveBeenCalledWith(expect.stringMatching(/\/test\/file\.txt\.[a-f0-9]+\.tmp$/), '/test/file.txt');
            });
            ...
            it('applies multiple edits sequentially', async () => {
                ...
                expect(mockFs.writeFile).toHaveBeenCalledWith(expect.stringMatching(/\/test\/file\.txt\.[a-f0-9]+\.tmp$/), 'first line\nline2\nthird line\n', 'utf-8');
                expect(mockFs.rename).toHaveBeenCalledWith(expect.stringMatching(/\/test\/file\.txt\.[a-f0-9]+\.tmp$/), '/test/file.txt');
            });
            ...
            it('handles complex multi-line edits with indentation', async () => {
                ...
                expect(mockFs.writeFile).toHaveBeenCalledWith(expect.stringMatching(/\/test\/file\.js\.[a-f0-9]+\.tmp$/), 'function test() {\n  console.log("world");\n  console.log("test");\n  return false;\n}', 'utf-8');
                expect(mockFs.rename).toHaveBeenCalledWith(expect.stringMatching(/\/test\/file\.js\.[a-f0-9]+\.tmp$/), '/test/file.js');
            });
            it('handles edits with different indentation patterns', async () => {
                ...
                mockFs.rename.mockResolvedValueOnce(undefined);
                await applyFileEdits('/test/file.js', edits, false);
                expect(mockFs.writeFile).toHaveBeenCalledWith(expect.stringMatching(/\/test\/file\.js\.[a-f0-9]+\.tmp$/), '    if (condition) {\n        doSomethingElse();\n        doAnotherThing();\n    }', 'utf-8');
                expect(mockFs.rename).toHaveBeenCalledWith(expect.stringMatching(/\/test\/file\.js\.[a-f0-9]+\.tmp$/), '/test/file.js');
            });
